...
from models import User, RentalCar
...
def createUser():
    ...
    session.query(User).filter_by(firstName=data['firstName']).first()
    ...
    users = User( username=data['username'],firstName=data['firstName'], lastName=data['lastName'], Phone=data['phone'],password=hashpassword,
                 role=data['role'])
    session.add(users)
    session.commit()
    session.close()
...
def getUserById(username):
    ...
    biblethump = {'id': username.id, 'username': username.username,'firstName': username.firstName, 'lastName': username.lastName, 'phone': username.phone}
...
def updateUser(current_user, id):
    ...
    if 'lastName' in data.keys():
        session.query(User).filter_by(lastName=data['lastName']).first()
        user_data.lastName = data['lastName']

    if 'phone' in data.keys():
        session.query(User).filter_by(Phone=data['phone']).first()
...
def deleteUser(current_user, id):
    ...
    if not id:
        return Response(status=404, response="Id doesn't exist")
    exist = session.query(RentalCar).filter_by(user_id=id.id).first()
    if exist:
        return Response(status=400, response="This user is a foreign key")
    session.delete(id)
    session.commit()
    session.close()
    return Response(response="User has been successfully deleted")
